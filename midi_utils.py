...
def midi_to_samples(file_name, encode_length=False, num_notes=96, samples_per_measure=96):
    ...
    for track in mid.tracks:
        for msg in track:
            if msg.type == 'time_signature':
                # adapt ticks per measure of this specific song
    ...
    # all_notes format: { pitch# : [start, stop, velocity] }
    all_notes = {}
    note_velocities = {}
    for track in mid.tracks:
            ...
            if msg.type == 'note_on':
                ...
                if note < 0 or note >= num_notes:  # ignore a note that is outside of that range
                    print('Ignoring', file_name,
                          'note is outside 0-%d range' % (num_notes - 1))
                    return []
                ...
                if note not in all_notes:
                    ...
                    if len(single_note) == 1:
                        single_note.append(single_note[0][0] + 1)

                # store the time a note has been played
                all_notes[note].append(
                    [[abs_time * samples_per_measure / ticks_per_measure, float(msg.velocity)]])

            # if a note ends
            elif msg.type == 'note_off':

                # if the note has already ended before (note_on, note_off, yeahnote_off), we skip the event
    ...
    for note in all_notes:
        for start_end in all_notes[note]:
            if len(start_end) == 1:
                start_end.append(start_end[0][0] + 1)

    # flatten each list in elements of all_notes and swap the velocity with the stop time
    i = 0  # list index
    for note in all_notes:
        for info_list in all_notes[note]:

            # replace the info list of each note with a flattened version of itself
            all_notes[note][i] = list(flatten(info_list))

            # swap 2nd and 3rd elements of each flattened list
            all_notes[note][i][1], all_notes[note][i][2] = all_notes[note][i][2], all_notes[note][i][1]
...
def samples_to_midi(samples, file_name, threshold=0.5, num_notes=96, samples_per_measure=96):
    ...
    program_message = Message(
        'program_change', program=piano, time=0, channel=0)
